...
## 
## $Header: /devl/xcs/repo/env/Jobs/MDT/sw/Apps/debug/new_xmd/DataFiles/Attic/genace.tcl,v 1.1.2.20 2010/11/03 15:23:28 rajn Exp $
###############################################################################
...
# Production SP605
set sp605(jtag_chain_options) "-configdevice devicenr 1 idcode 0x04028093 irlength 6 partname xc6slx45t"
set sp605(jtag_fpga_position) 1
set sp605(jtag_devices) "xc6slx45t"

# Production ML605
set ml605(jtag_chain_options) "-configdevice devicenr 1 idcode 0x04250093 irlength 10 partname xc6vlx240t"
set ml605(jtag_fpga_position) 1
set ml605(jtag_devices) "xc6vlx240t"

# Production XUPV5-LX110T
set xupv5(jtag_chain_options) "-configdevice devicenr 1 idcode 0xa2ad6093 irlength 10 partname xc5vlx110t" 
set xupv5(jtag_fpga_position) 1
set xupv5(jtag_devices) "xc5vlx110t"
...
proc impact_bit2svf { bitfile jtag_fpga_position jtag_devices } {
	...
	if { ![string match "*Programming completed successfully.*" $msg] } {
	    puts $msg
...
proc write_swsuffix { target_type elffile xmd_options saddr } {
    ...
    xdebugconfig $tgt -reset_on_run enable

	xreset $tgt -processor
    xwreg $tgt 32 [format "0x%x" $saddr]

    xcontinue $tgt [format "0x%x" $saddr]
    # Single MB systems in 10.1 SP3 do not appear to need this
    # This is a temporary fix which will be removed in later release
#     if { $target_type != "ppc_hw" } {
#         xreset $tgt -processor
#     }
...
proc impact_svf2ace { acefile } {
    ...
    # Spartan6 requires the ACE controller to be slowed down
    set fpga_family  [expr { ($fpga_idcode & 0x0fe00000) >> 21 }]
    if { $fpga_family == 0x20 } {
       puts $script "svf2ace -tck 33e6 -wtck -d -m 16776192 -i $svffile -o $acefile"
    } else {
...
# V6 requires extra delay for MIG calibration
set delayTCK 2000000
set fpga_family  [expr { ($fpga_idcode & 0x0fe00000) >> 21 }]
if { $fpga_family == 0x21 } {
	set delayTCK 2000000
}
...
proc genace_hw { } {
	...
	# Move the SVF to the Current directory
	file rename -force [file rootname $bitfile].svf [pwd]
    }
}
...
proc genace_sw_download { } {
	...
	while { $i < [llength $data_files] } {
	    ...
	    # Move the SVF to the Current directory
	    file rename -force [file rootname $dfile].svf [pwd]
	}
    }
	...
	foreach elf $elf_files { 
	    ...
	    # Move the SVF to the Current directory
	    file rename -force [file rootname $elf].svf [pwd]
	}
    }
}
...
set ace_prefix [file tail $param(ace)]
set ace_prefix [file rootname $ace_prefix]
set flist [concat $param(hw) $flist]
foreach filename $flist {
    set tfilename [file tail $filename]
    set fname_prefix [file rootname $tfilename]
...
# Get the IDCODE of FPGA.
set options_list [concat $param(jtag_chain_options)]
set index $param(jtag_fpga_position)
set i 0
while { $i < [llength $options_list] } {
    if {[string match -nocase "idcode" [lindex $options_list $i]]} {
        incr index -1
        if { $index == 0 } {
            set fpga_idcode [lindex $options_list [expr $i+1]]
            break
        }
    }
    incr i 1
...
if { $index > 0 } {
    error "Cannot find FPGA IDCode.. Please check your JTAG options"
}
